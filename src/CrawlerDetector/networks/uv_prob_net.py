...
class uvProbNet(NetworkBase):
    def __init__(self, num_nc=32):
        super(uvProbNet, self).__init__()
        self._name = 'uvProbNet'

        features_cfg = [num_nc, num_nc, 'M', 2*num_nc, 2*num_nc, 'M', 4*num_nc, 4*num_nc, 4*num_nc, 'M']
        pose_cfg = [6*num_nc, 6*num_nc, 6*num_nc, 'M', 6*num_nc, 6*num_nc, 6*num_nc, 'M']
        prob_cfg = [6*num_nc, 6*num_nc, 'M', 6*num_nc, 6*num_nc, 'M']

        self._features = self._make_layers(features_cfg, 3, batch_norm=False)
    ...
    def forward(self, x):
        ...
        pose = self._pose_reg(self._pose_conv(features).view(x.size(0), -1))
        prob = self._prob_reg(self._pose_conv(features).view(x.size(0), -1))
        return pose, prob